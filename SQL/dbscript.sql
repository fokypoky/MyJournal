...
VALUES (1, 1, 'Страница 190 задачи 11-15', 1),
       (2, 1, 'Электричество и магнитизм', 1),
       (3, 1, 'Функции и методы C#, перегрузка, рекурсия', 1);
-- добавляю оценки
...
SELECT * FROM Tasks t JOIN Classes C on C.id = t.class_id
        JOIN Subjects ON t.subject_id = Subjects.id
...
UPDATE Contacts SET userrole_id = 1 WHERE ID < 6;
UPDATE Contacts SET userrole_id = 2 WHERE ID > 5 AND ID < 11;
UPDATE Contacts SET userrole_id = 3 WHERE ID > 10 AND ID <= 15;
...
UPDATE Marks SET subject_id = 1 WHERE task_id = 1;
UPDATE Marks SET subject_id = 2 WHERE task_id = 2;
UPDATE Marks SET subject_id = 3 WHERE task_id = 3;
...
SELECT * FROM Marks m JOIN Tasks T on m.task_id = T.id WHERE m.mark = 2;
SELECT * FROM Subjects;

SELECT * FROM Tasks WHERE subject_id = 2 AND class_id = 1;
SELECT * FROM Marks WHERE task_id = 2;
SELECT * FROM Marks m JOIN Subjects s ON m.subject_id = s.id;
...
INSERT INTO Tasks(subject_id, class_id, task_text, teacher_id, task_start_date, task_deadline_date)
    VALUES(2, 1, 'Молекулярная физика. Как спариваются микробы?', 1, '2023-07-18', '2023-07-25')

INSERT INTO Marks(student_id, mark, mark_date, teacher_id, task_id, subject_id)
    VALUES(1, 4, '2023-07-18', 1, 5, 2)
